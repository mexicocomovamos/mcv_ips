...
# Objetivo:             Homicidios (Tasa de homicidios por cada 100mil habitantes)
#
# 
...
# 
# Fecha de creación:    11 de julio de 2023
# Última actualización: 11 de julio de 2023
#------------------------------------------------------------------------------#

# Fuente:https://www.inegi.org.mx/sistemas/olap/proyectos/bd/continuas/mortalidad/defuncioneshom.asp?s=est
...
df_crudo    <- read_excel(paste_inp("INEGI_exporta_homicidios.xlsx"), skip = 4)
...
df_homicidios <- df_final 

save(df_homicidios, 
     file = paste_out("df_homicidios.RData"))

ggplot(df_limpio %>% filter(entidad == "Nacional"), 
       aes(x = anio, y = total, group = entidad)) + 
  geom_line()

ggplot(df_homicidios %>% filter(entidad == "Nacional"), 
