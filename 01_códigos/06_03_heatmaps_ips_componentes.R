...
ggplot(
    ...
    # Geoms
    geom_tile(col = "white") +
    geom_text(aes(label = round(value, 2)), family = "Ubuntu, size = 4") +
    # Etiquetas
    labs(
        title = str_wrap(v_name, 25), 
        subtitle = v_years, 
        x = "Años", 
        y = v_empty, 
        color = v_empty
...
ggsave(
    filename = paste_fig(
        paste0(
            v_id, "_", str_replace_all(tolower(trimws(str_remove_all(v_name, "[[:punct:]]"))), " ", "_"), ".png")), 
    width = 12, height = 23, dpi = 200)

ggsave(
    filename = paste_fig(
        paste0(
            v_id, "_", str_replace_all(tolower(trimws(str_remove_all(v_name, "[[:punct:]]"))), " ", "_"), ".svg")), 
    width = 12, height = 23, dpi = 200)
